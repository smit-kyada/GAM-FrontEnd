...
import { UPDATE_MESSAGE } from 'src/graphql/mutation/message'
...
const UpdateMessage = ({ userData, open, toggle, messageRes }) => {


  // ** Graphql
  const [updateMessage] = useMutation(UPDATE_MESSAGE);

  // const defaultImg = userData?.image && `${process.env.NEXT_PUBLIC_BASE_URL}/${userData?.image}`
  // const defaultIcon = userData?.icon && `${process.env.NEXT_PUBLIC_BASE_URL}/${userData?.icon}`

  const defaultIcon = userData?.icon.includes('http') ? "" : `${process.env.NEXT_PUBLIC_BASE_URL}/${userData?.icon}`
  const defaultItype = userData?.icon.includes('http') ? "Url" : "Base64"

  const defaultImg = userData?.image.includes('http') ? "" : `${process.env.NEXT_PUBLIC_BASE_URL}/${userData?.image}`
  const defaultBtype = userData?.image.includes('http') ? "Url" : "Base64"
  ...
  const [itype, setItype] = useState(defaultItype)
  ...
  const handleInputImgChange = file => {
    ...
    if (files && files.length !== 0) {
      convertToBase64(files[0]).then((result) => setImgBase64(result))
      reader.onload = () => {
  ...
  const defaultValues = {
    ...
    burl: userData?.image.includes('http') ? userData?.image : "",
    iurl: userData?.icon.includes('http') ? userData?.icon : ""
  }
  ...
  const onSubmit = data => {
    ...
    if (itype === "Base64" && iconbase64) {
      Obj.icon = iconbase64
      Obj.oldIcon = userData?.icon
    }
    if (itype === "Url") {
      Obj.icon = data?.iurl
      Obj.oldIcon = userData?.icon.includes("http") ? "" : userData?.icon
    }
    if (btype === "Base64" && imgbase64) {
      Obj.image = imgbase64
      Obj.oldImage = userData?.image
    }
    if (btype === "Url") {
      Obj.image = data?.burl
      Obj.oldImage = userData?.image.includes("http") ? "" : userData?.image
    }

    // if (imgbase64) {
    //   data.image = imgbase64;
    //   data.oldImage = userData?.image
    // }

    // if (iconbase64) {
    //   data.icon = iconbase64;
    //   data.oldIcon = userData?.icon
    // }

    updateMessage({ variables: { input: Obj } }).then((result) => {
      setDisable(false);
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Update Message</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
