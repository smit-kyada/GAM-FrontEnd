...
import { ADD_SUBADMIN, ADD_USER } from 'src/graphql/mutation/user'
...
const AddSubAdmin = props => {
  ...
  // ** Props
  const { open, toggle, subAdminRefetch } = props
  ...
  // ** Graphql
  const [addSubadmin] = useMutation(ADD_USER);
  ...
  const onSubmit = data => {
    ...
    userId?.length > 0 && (data.users = userId)
    data.role = "subadmin"
  ...
  return (
    ...
    >
      <Header>
        <Typography variant='h6'>Add Sub admin</Typography>
        <IconButton
          size='small'
          onClick={handleClose}
