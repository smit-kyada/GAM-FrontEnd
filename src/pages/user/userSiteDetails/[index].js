...
const SiteDetail = () => {
  ...
  const { loading: siteTableLoading, error: siteTableError, data: siteTableData, refetch: siteTableRefetch } = useQuery(GET_SITETABLE, {
    variables: { ...Obj, fetchAll: false },
    fetchPolicy: "cache-and-network",
  });

  const { loading: siteTableLoadingAll, error: siteTableErrorAll, data: siteTableDataAll, refetch: siteTableRefetchAll } = useQuery(GET_SITETABLE, {
    variables: { ...Obj, fetchAll: true },
    fetchPolicy: "cache-and-network",
  });
  ...
  useEffect(() => {
    ...
    siteTableData?.getSiteTable?.data && setDataAll(siteTableDataAll?.getSiteTable?.data)
  ...
  }, [siteTableData, siteTableDataAll])
      ...
      <Grid item xs={12}>
        <Card sx={{ mt: 5 }}>
          ...
          <TableHeader startDate={startDate} setStartDate={setStartDate} endDate={endDate}
            setEndDate={setEndDate} toggle={toggleAddUserDrawer} userId={Obj?.userId} dataAll={dataAll} />
          <Divider sx={{ m: '0 !important' }} />
          <DataGrid
            autoHeight
